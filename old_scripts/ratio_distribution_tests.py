...
ratio_samples = [
    x / y for x, y in zip(
        np.random.normal(x_mu, scale_x, size=N_SAMPLES),
        np.random.normal(y_mu, scale_y, size=N_SAMPLES)
    )
]
...
mean_new_ratio_samples = np.mean(new_ratio_samples)
new_ratio_samples = [ mean_new_ratio_samples for i in range(len(new_ratio_samples)) ]
...
# recovered_gaussian = [
#     z * y for z, y in zip(
#         new_ratio_from_mean_samples, np.random.normal(y_mu, scale_y, size=N_SAMPLES)
#     )
# ]
...
plt.hist(
    ratio_samples_mean * np.random.normal(y_mu, scale_y, size=N_SAMPLES), bins=BINS, color='r',
    histtype='step'
)

ratio_samples_proper_mean = (
    np.mean(np.random.normal(x_mu, scale_x, size=N_SAMPLES)) *
    np.mean([ 1 / y for y in np.random.normal(y_mu, scale_y, size=N_SAMPLES) ])
)

plt.hist(
    ratio_samples_proper_mean * np.random.normal(y_mu, scale_y, size=N_SAMPLES), bins=BINS,
